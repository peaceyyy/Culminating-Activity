...
imp_df <- base_df %>%
  ...
  select(Salary, Uses_AI, Job_Satisfaction, TimeSearching)
...
run_imputation_eval <- function(data, numeric_method, label) {
  ...
  results <- lapply(1:imp$m, function(i) {
    ...
    calc_metrics(completed, formula, label, "model_3_satisfaction")
  })
...
write_csv(summary_results, "outputs/model summaries/all/model_3_satisfaction_summary.csv")
cat("\nSaved summary to outputs/model summaries/all/model_3_satisfaction_summary.csv\n")

cat("\n======================================================\n")
cat("MODEL 3: THE SATISFACTION TRADEOFF (Dependent: Job_Satisfaction)\n")
cat("======================================================\n")
