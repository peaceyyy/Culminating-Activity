...
df <- read_csv("outputs/data_imputed_cart.csv", show_col_types = FALSE) %>%
  mutate(
...
calc_metrics <- function(data, formula) {
  ...
  s <- summary(fit)
  tibble(
    imputation = "cart_imputed",
    model = "model_2_productivity",
...
dir.create("outputs/model summaries/cart", recursive = TRUE, showWarnings = FALSE)
write_csv(summary_results, "outputs/model summaries/cart/model_2_productivity_summary.csv")

cat("\n======================================================\n")
cat("MODEL 2: Dependent - TimeSearching_Min)\n")
cat("======================================================\n")
