...
The dataset was checked with [check_data.r](check_data.r) to review missingness patterns, unique-value counts, and basic distributions in both the raw survey data and the filtered modeling table.

Exploratory visualization was handled with [eda.rmd](eda.rmd) to inspect the early relationships among AI adoption, salary, job satisfaction, experience, and time spent searching/debugging. This step served as a plausibility check before modeling.
...
In this study the lowest observed `xerror` across the tested seeds occurred at `set.seed(200)` (xerror ≈ 0.9784), so the main results are reported using the imputed dataset generated with `set.seed(200)`. The full seed-run results are available in [outputs/cart_mice_seed_runs.csv](outputs/cart_mice_seed_runs.csv) and summarized in [outputs/cart_mice_seed_averages.csv](outputs/cart_mice_seed_averages.csv).
...
Recommendation: report the seed used for the published baseline, include the full seed-sensitivity results (as found in [outputs/cart_mice_seed_averages.csv](outputs/cart_mice_seed_averages.csv)), and treat the seed choice as part of the reproducibility metadata rather than a hyperparameter that needs theoretical justification.
