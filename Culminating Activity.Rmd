...
style_simple_table(cp_profile, caption = "Cost-complexity pruning profile")
```
...
style_simple_table(best_shuffle, caption = "Best hyperparameter setting")
```
...
seed_table <- seed_averages %>% arrange(xerror)
style_simple_table(seed_table, caption = "Seed sensitivity summary")
```
...
The robustness checks reinforce the tree and importance results. When the model settings are shuffled, the same pattern stays visible: the chosen tree still favors TimeSearching_Min under the best-performing settings, while AI usage remains weak and unstable. When the missing data is perturbed through the seed sensitivity check, the same broad conclusion holds.
...
```{r appendix-descriptive-statistics, eval=FALSE}
library(dplyr)
library(readr)
library(knitr)
...
```{r appendix-cart-tree-figure, echo=FALSE, message=FALSE, warning=FALSE}
library(rpart)
...
```{r appendix-variable-importance, eval=FALSE}
importance_rank <- c(
  TimeSearching_Min = 2660.7569,
  Years_Exp = 783.4335,
  Salary = 214.9234
...
```{r appendix-cp-profile-table, eval=FALSE}
cp_profile <- read_csv("outputs/cart_cp_profile.csv", show_col_types = FALSE)

cp_profile
```

```{r appendix-cp-profile-visual, eval=FALSE}
plot(
  cp_profile$CP,
  cp_profile$xerror,
  type = "b",
...
```{r appendix-hyperparam-shuffling-table, eval=FALSE}
shuffle <- read_csv("outputs/cart_hyperparam_shuffle.csv", show_col_types = FALSE)
...
```{r appendix-hyperparam-shuffling-visual, eval=FALSE}
valid_shuffle <- shuffle %>%
  filter(status == "ok", !is.na(cp), !is.na(xerror), !is.na(nsplit))
...
```{r appendix-seed-sensitivity-table, eval=FALSE}
seed_averages <- read_csv("outputs/cart_mice_seed_averages.csv", show_col_types = FALSE)

seed_averages %>% arrange(xerror)
```

```{r appendix-seed-sensitivity-visual, eval=FALSE}
imp_vars <- c("TimeSearching_Min", "Years_Exp", "Salary", "Uses_AI", "Is_Remote")
seed_plot <- seed_averages %>%
  select(seed, all_of(imp_vars)) %>%
  arrange(seed)
