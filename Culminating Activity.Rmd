...
author: "Aloba, Loyan Edrian | Delrio, Martin Nicholas | Dorin, Homer Adriel | Rivera, Nythan"
...
summary_df <- df %>%
  summarise(
    n = n(),
    JS_Mean     = mean(Job_Satisfaction,   na.rm = TRUE),
    JS_SD       = sd(Job_Satisfaction,     na.rm = TRUE),
    Salary_Mean = mean(Salary,             na.rm = TRUE),
    Salary_SD   = sd(Salary,               na.rm = TRUE),
    TimeSearch_Mean = mean(TimeSearching_Min, na.rm = TRUE),
    TimeSearch_SD   = sd(TimeSearching_Min,   na.rm = TRUE),
    YrsExp_Mean = mean(Years_Exp,          na.rm = TRUE),
    YrsExp_SD   = sd(Years_Exp,            na.rm = TRUE),
    AI_Rate     = mean(as.numeric(as.character(Uses_AI))  == 1, na.rm = TRUE),
    Remote_Rate = mean(as.numeric(as.character(Is_Remote)) == 1, na.rm = TRUE)
  )

# Split into two display rows so it fits on the page
row1 <- summary_df %>% select(n, JS_Mean, JS_SD, Salary_Mean, Salary_SD, TimeSearch_Mean, TimeSearch_SD)
row2 <- summary_df %>% select(YrsExp_Mean, YrsExp_SD, AI_Rate, Remote_Rate) %>%
  rename(
    `YrsExp Mean` = YrsExp_Mean,
    `YrsExp SD`   = YrsExp_SD,
    `AI Adoption Rate` = AI_Rate,
...
table1 <- knitr::kable(
  row1,
  digits  = 2,
  caption = "Descriptive summary (Part 1 of 2)",
  booktabs = TRUE,
  col.names = c("n", "JS Mean", "JS SD", "Salary Mean", "Salary SD",
                "TimeSearch Mean", "TimeSearch SD")
)
if (has_kableextra) {
  table1 <- table1 %>%
...
table2 <- knitr::kable(
  row2,
  digits   = 2,
  caption  = "Descriptive summary (Part 2 of 2)",
  booktabs = TRUE
)
if (has_kableextra) {
  table2 <- table2 %>%
...
table3 <- knitr::kable(
  head(df),
  digits   = 2,
  caption  = "Sample first 6 rows of data (imputed using CART)",
  booktabs = TRUE
)
if (has_kableextra) {
  table3 <- table3 %>%
...
```{r cp-profile-visual, echo=FALSE}
plot(
  cp_profile$CP,
  cp_profile$xerror,
  type = "b",
  pch = 19,
  col = "#2F5AA8",
  xlim = rev(range(cp_profile$CP)),
  xlab = "Complexity Parameter (CP)",
  ylab = "Cross-Validated Error (xerror)",
  main = "Cost-Complexity Pruning Profile"
)

abline(v = 0.001, lty = 2, lwd = 2, col = "#C0392B")
text(0.001, max(cp_profile$xerror), labels = "cp = 0.001", pos = 4, cex = 0.8, col = "#C0392B")
```
...
```{r hyperparam-shuffling-visual, echo=FALSE}
valid_shuffle <- shuffle %>%
  filter(status == "ok", !is.na(cp), !is.na(xerror), !is.na(nsplit))

max_splits <- max(valid_shuffle$nsplit, na.rm = TRUE)
point_cex <- ifelse(
  valid_shuffle$nsplit == 0,
  1.35,
  0.75 + (valid_shuffle$nsplit / max_splits) * 1.7
)
point_col <- ifelse(valid_shuffle$nsplit == 0, "#C0392B", "#2F5AA8")

plot(
  valid_shuffle$cp,
  valid_shuffle$xerror,
  pch = 19,
  cex = point_cex,
  col = point_col,
  xlab = "Pruning Strength (cp)",
  ylab = "Cross-Validated Error (xerror)",
  main = "Hyperparameter Shuffling: xerror by cp (size = nsplit)"
)

abline(v = 0.001, lty = 2, lwd = 2, col = "#16A085")
text(0.001, max(valid_shuffle$xerror), labels = "cp = 0.001", pos = 4, cex = 0.8, col = "#16A085")

legend(
  "topright",
  legend = c("nsplit > 0", "nsplit = 0 (over-pruned)", "larger point = more splits"),
  pch = c(19, 19, 19),
  pt.cex = c(1.2, 1.2, 2.0),
  col = c("#2F5AA8", "#C0392B", "#2F5AA8"),
  bty = "n",
  cex = 0.8
)
```
...
```{r cart-tree-visual, echo=FALSE, message=FALSE, warning=FALSE}
library(rpart)
...
The final model's variable importance ranking gives a simple summary of the same pattern seen in the tree and robustness checks. `TimeSearching_Min` is the strongest predictor, followed by `Years_Exp`, then `Salary`, while `Uses_AI` remains weak or absent in the strongest runs (see the model output in `outputs/main_terminal_outputs.txt`).
